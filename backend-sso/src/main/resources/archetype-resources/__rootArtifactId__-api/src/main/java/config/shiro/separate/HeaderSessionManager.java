...
public class HeaderSessionManager extends DefaultSessionManager implements WebSessionManager
{
    ...
    private Serializable getReferencedSessionId(ServletRequest request, ServletResponse response)
    {
        ...
        // 如果cookie中没有session那么在header里面查找
        if (null == id)
        {
            id = getSessionIdHeaderValue(request, response);
        }
        if (id != null)
        {
     ...
     *
     * @param session the session that was just {@link ${symbol_pound}createSession created}.
     */
    @Override
